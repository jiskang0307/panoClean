"""
debug_blur_down.py — down face blur 처리 확인.

저장:
  debug_output/blur_down_result.jpg  — before / blur결과 / diff 나란히
  debug_output/result_erp.jpg        — 전체 ERP 재합성 결과

실행:
    python scripts/debug_blur_down.py
"""
...
inpainter = LamaInpainter(
    ...
    down_blur_kernel=151,
    down_blur_feather=61,
)
...
photo_mask = seg_down["photographer_mask"]   # (512,512) bool
...
print(f"blur 처리: {did_process}")

# ── 시각화: before / after / diff ─────────────────────────────────────────
...
for img, label in [(before_bgr, "before"), (after_bgr, "after (blur)"), (diff_vis, "diff x3")]:
    cv2.putText(img, label, (8, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

panel = np.hstack([before_bgr, after_bgr, diff_vis])
out1  = DEBUG_OUT / "blur_down_result.jpg"
...
out2 = DEBUG_OUT / "result_erp.jpg"
save_erp(result_erp, str(out2))
print(f"저장: {out2}  ({out2.stat().st_size // 1024} KB)")

# nadir crop (하단 25%)
erp_bgr = cv2.imread(str(out2))
