"""
debug_mask_ellipse_v2.py — down face 타원 mask v2 확인.

실행:
    python scripts/debug_mask_ellipse_v2.py

저장:
    debug_output/mask_ellipse_down_v2.jpg     — hull / 타원(중앙고정) / 최종OR
    debug_output/mask_ellipse_down_v2_seg.jpg — segment_face() 실제 출력
"""
...
# ── hull mask 생성 (_postprocess_photographer_mask 결과) ──────────────────
...
if photo_dets:
    ...
    print(f"  YOLO combined bbox : {bbox}")
else:
...
print(f"  타원 center        : ({cx}, {cy})  [face 정중앙]")
print(f"  타원 axes          : ({ax_w}, {ax_h})  [{AX_RATIO:.2f}×W, {AX_RATIO:.2f}×H]")
...
cv2.drawMarker(vis_seg, (cx, cy), (0, 0, 255), cv2.MARKER_CROSS, 24, 3)
cnts_e2, _ = cv2.findContours(ellipse_np, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
cv2.drawContours(vis_seg, cnts_e2, -1, (0, 220, 220), 1)
if bbox:
    x1, y1, x2, y2 = bbox
...
label(vis_seg,
      "segment_face() result",
      f"center=({cx},{cy})  axes=({ax_w},{ax_h})",
      f"px={int((seg_np>0).sum())}",
      "청록=YOLO bbox  청색=타원경계  십자=중앙")

out2 = DEBUG_OUT / "mask_ellipse_down_v2_seg.jpg"
